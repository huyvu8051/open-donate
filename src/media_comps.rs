            ...
            <div class="bg-surface-variant/20 p-4 rounded-xl border border-white/5">
                ...
                <ActionForm action=upload_action enctype="multipart/form-data">
                    <div class="flex flex-col gap-2">
                        ...
                        <button type="submit" class=move || format!("bg-primary text-on-primary font-bold px-4 py-2 rounded-xl mt-2 w-fit {}", if upload_action.pending().get() { "opacity-50" } else { "" }) disabled=move || upload_action.pending().get()>
                            "Upload File (Max 2MB)"
                        </button>
                    </div>
                </ActionForm>
